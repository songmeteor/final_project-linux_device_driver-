...
static const unsigned char font5x7[] = {
	...
	0x00, 0x00, 0x7F, 0x00, 0x00,0x00, 0x41, 0x36, 0x08, 0x00,0x02, 0x01, 0x02, 0x01, 0x00
};
static const unsigned char icon_prev[] = {0x1C, 0x3C, 0x7C, 0xFC, 0xFC, 0x7C, 0x3C, 0x1C};
static const unsigned char icon_play[] = {0x18, 0x3C, 0x7E, 0xFF, 0xFF, 0x7E, 0x3C, 0x18};
static const unsigned char icon_pause[] = {0xFF, 0xFF, 0xDB, 0xDB, 0xDB, 0xDB, 0xFF, 0xFF};
static const unsigned char icon_next[] = {0x38, 0x3C, 0x3E, 0x3F, 0x3F, 0x3E, 0x3C, 0x38};
static const unsigned char icon_repeat[] = {0x0C, 0x1E, 0x33, 0x21, 0x42, 0x66, 0x3C, 0x18};

// --- I2C 및 화면 제어 함수 ---
static int oled_send_cmd(u8 cmd) { return i2c_smbus_write_byte_data(oled_client, 0x00, cmd); }
...
static void draw_char(int x, int y, char c) {
    int i;
    const unsigned char *font = &font5x7[(unsigned char)(c - 32) * FONT_WIDTH];
    if (x > OLED_WIDTH - FONT_WIDTH || y > OLED_HEIGHT - 8) return;
...
static void draw_icon(int x, int y, const unsigned char *icon) {
    int i;
    if (x > OLED_WIDTH - 8 || y > OLED_HEIGHT - 8) return;
    for (i = 0; i < 8; i++) {
...
static void oled_display_init(void) {
    msleep(100);
    oled_send_cmd(0xAE); oled_send_cmd(0xD5); oled_send_cmd(0x80);
    oled_send_cmd(0xA8); oled_send_cmd(OLED_HEIGHT - 1);
    oled_send_cmd(0xD3); oled_send_cmd(0x00); oled_send_cmd(0x40 | 0x0);
    oled_send_cmd(0x8D); oled_send_cmd(0x14); oled_send_cmd(0x20);
    oled_send_cmd(0x00); oled_send_cmd(0xA1); oled_send_cmd(0xC8);
    oled_send_cmd(0xDA); oled_send_cmd(0x12); oled_send_cmd(0x81);
    oled_send_cmd(0xCF); oled_send_cmd(0xD9); oled_send_cmd(0xF1);
    oled_send_cmd(0xDB); oled_send_cmd(0x40); oled_send_cmd(0xA4);
    oled_send_cmd(0xA6); oled_send_cmd(0xAF);
...
static long oled_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    if (cmd == OLED_UPDATE_UI) {
        if (copy_from_user(&ui_data, (void __user *)arg, sizeof(ui_data))) return -EFAULT;
        ...
        if (ui_data.play_state == 1) draw_icon(60, 56, icon_play);
        else draw_icon(60, 56, icon_pause);
...
static int __init oled_driver_init(void) {
    ...
    struct i2c_board_info board_info = { .type = "ssd1306", .addr = OLED_I2C_ADDR };
    if (alloc_chrdev_region(&oled_dev_num, 0, 1, OLED_DEVICE_NAME) < 0) return -1;
    cdev_init(&oled_cdev, &oled_fops);
    if (cdev_add(&oled_cdev, oled_dev_num, 1) < 0) goto unreg_chrdev;
    oled_class = class_create(OLED_CLASS_NAME);
    if (IS_ERR(oled_class)) goto del_cdev;
    oled_device = device_create(oled_class, NULL, oled_dev_num, NULL, OLED_DEVICE_NAME);
    if (IS_ERR(oled_device)) goto destroy_class;
    adapter = i2c_get_adapter(1);
    if (!adapter) goto destroy_device;
    oled_client = i2c_new_client_device(adapter, &board_info);
    i2c_put_adapter(adapter);
    if (!oled_client) goto destroy_device;
    oled_display_init();
    pr_info("OLED MP3 UI driver initialized\n");
    return 0;

destroy_device: device_destroy(oled_class, oled_dev_num);
destroy_class: class_destroy(oled_class);
del_cdev: cdev_del(&oled_cdev);
unreg_chrdev: unregister_chrdev_region(oled_dev_num, 1);
    return -1;
}
